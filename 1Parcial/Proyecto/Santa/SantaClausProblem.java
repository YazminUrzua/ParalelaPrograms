        ...
        public void run() {
            while (!endOfTheWorld) {
                try {
                   ...
                   System.out.printf("-- Reindeer %d arrived from vacations(%d) --\n", id, reindeerCount+1);
                    // increment of reindeers
                    reindeerCount++;
                    if (reindeerCount == NUM_REINDEERS_IN_GROUP) {
                        System.out.printf("** All Reindeers are complete for Christmas **\n");
    ...
    class Elf implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                while (!endOfTheWorld) {
                    elfMutex.acquire();
                    System.out.printf("Elf %d needs help (%d)\n", id, elfCount+1);
                    counterMutex.acquire();
                    elfCount++;
        ...
        public void run() {
            while (!endOfTheWorld) {
                try {
                    ...
                    System.out.printf("\n** Santa wakes up **\n");
                    if (reindeerCount == NUM_REINDEERS_IN_GROUP) {
                         System.out.printf("** Santa distributes gifts **\n"); 
